...
__global__ void reduceCompleteUnroll8Warps(int *g_idata, int *g_odata, unsigned int n)
{
    unsigned int tid = threadIdx.x;
    unsigned int idx = 8 * blockDim.x * blockIdx.x + threadIdx.x;
...
int main()
{
    ...
    // launch the kernel reduce complete unroll 8 warps
    cudaMemcpy(d_idata, h_idata, nBytes, cudaMemcpyHostToDevice);
    unroll = 8;
    printf("Execution reduceCompleteUnroll8Warps config <<<%d,%d>>>\n", grid.x / unroll, block.x);
    reduceCompleteUnroll8Warps<<<grid.x / unroll, block>>>(d_idata, d_odata, n);
    cudaDeviceSynchronize();
    ...
    if (!(cpu_sum == gpu_sum))
        printf("Test Failed for reduce complete Unroll8Warps");

    // launch the kernel template reduce complete unroll 8 warps
    cudaMemcpy(d_idata, h_idata, nBytes, cudaMemcpyHostToDevice);
    unroll = 8;
    printf("Execution reduceCompleteUnroll with template config <<<%d,%d>>>\n", grid.x / unroll, block.x);
    reduceCompleteUnroll<512><<<grid.x / unroll, block>>>(d_idata, d_odata, n);
    cudaDeviceSynchronize();
    ...
    if (!(cpu_sum == gpu_sum))
        printf("Test Failed for reduce complete Unroll8Warps with template");
